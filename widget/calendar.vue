    ...
    <div class="calendar-custom">
        ...
        <div class="days-con">
            <ul class="days clearfix">
                ...
                <li v-for="item in days.now" :class="{'none': year == yearNow && month == monthNow &&  item > dayNow }" v-on:click="click(item)"
                    track-by="$index">
                    <b>{{item}}</b>
        ...
        <template v-if="url">
            <p class="statis">
                <span class="status green"></span> 正常出勤
                <b>{{fattnCount}}</b>天&nbsp;
                <span class="status red"></span> 迟到
                <b>{{flateCount}}</b>次&nbsp;
                <span class="status red"></span> 早退
                <b>{{fleaveCount}}</b>次&nbsp;
                <span class="status red"></span> 旷工
                <b>{{fabsenceCount}}</b>次
            </p>
        </template>
...
<script>
    module.exports = {
        ...
        ready: function () {
            console.log(this.clickFun,this.url)
            var self = this;
            // this.req();
            ...
            if (week == 0) {
                //0-6代表周日-周六，计算不便，所以对应成1-7代表周1-周日
                week = 7;
            }
        ...
        methods: {
            ...
            click: function (text) {
                ...
                if (et.className != 'none') {
                    ...
                    this.clickFun&&this.clickFun(date, this.fpersonId);
                    if (this.active) {
                        this.active.className = '';
                    }
                    this.active = et;
            ...
            monthView: function (year, month) {
                ...
                var daysAll = this.monthsDay(this.yearNow);
                var yearGap = this.yearNow - year;
                var length = yearGap * 12 + this.monthNow - month; //差多少个月
                var daysGap = 0, monthCount = this.monthNow, yearCount = this.yearNow;//总差距天数，差距月，差距年
                ...
                for (var i = 0; i < length; i++) {
                    ...
                    if (monthCount < 0) {
                        monthCount = 11;
                        yearCount--;
                        daysAll = this.monthsDay(yearCount);
                    }
                    daysGap = daysGap + daysAll[monthCount];
                }
                var firstWeekDay = daysGap - parseInt(daysGap / 7) * 7 + 1; //首星期的今天是几号，+1是因为本月1号，多一天
                if (firstWeekDay == 0) {
                    firstWeekDay = 7;
                }
                ...
                for (var m = 1; m < firstDayWeek; m++) {
                    //每年的12月都一样
                    dayDisplay.prev.push(daysAll[month2] - firstDayWeek + m + 1);
                }
